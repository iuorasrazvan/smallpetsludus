...
<script type="text/javascript">
   ...
   if (typeof (overlay_loading_contact_bank_<?php echo $random; ?>) !== "function")
   {
   function overlay_loading_contact_bank_<?php echo $random; ?>(message) {
   ...
   if (typeof (remove_overlay_contact_bank_<?php echo $random; ?>) !== "function")
   {
   function remove_overlay_contact_bank_<?php echo $random; ?>() {
   jQuery(".loader_opacity").remove();
   jQuery(".opacity_overlay").remove();
   }
   }
   if (typeof (only_characters_contact_bank_<?php echo $random; ?>) !== "function")
   {
   function only_characters_contact_bank_<?php echo $random; ?>(event, timestamp, max_limit, value) {
   ...
   if (typeof (only_digits_contact_bank_<?php echo $random; ?>) !== "function")
   {
   function only_digits_contact_bank_<?php echo $random; ?>(event, timestamp, max_limit, text_appear)
   {
   var content = jQuery("#ux_txt_singal_line_text_" + timestamp).val();
   ...
   if (typeof (apply_input_masking_contact_bank_<?php echo $random; ?>) !== "function")
   {
   function apply_input_masking_contact_bank_<?php echo $random; ?>(input_id, timestamp, input_mask_type, custom_mask_value) {
   ...
   if (typeof (change_label_placement_contact_bank_<?php echo $random; ?>) !== "function")
   {
   function change_label_placement_contact_bank_<?php echo $random; ?>(input_id, timestamp, label_placement_type, control_name)
   {
   ...
   if (typeof (number_settings_contact_bank_<?php echo $random; ?>) !== "function")
   {
   function number_settings_contact_bank_<?php echo $random; ?>(timestamp, event, step_number_cb, min_number, max_number)
   {
   var count = 0;
   var step_count = 0;
   ...
   if (typeof (submit_form_contact_bank_<?php echo $random; ?>) !== "function")
   {
   function submit_form_contact_bank_<?php echo $random; ?>(controls_ids, form_name, save_submssion, page_url, submission_enable_disable, submission_limit, redirect_type, redirect_url, form_success_message)
   {
<?php
if (isset($_REQUEST["cb_preview_form"])) {
   ?>
      var form_id = "<?php echo isset($_REQUEST["cb_preview_form"]) ? intval($_REQUEST["cb_preview_form"]) : ""; ?>";
   <?php
} else {
   ?>
...
?>
   ...
   jQuery("#ux_frm_main_container_<?php echo $random; ?>").validate({
      ...
      switch ($controls["required_type"]) {
         case "enable" :
            switch ($controls["control_type"]) {
               case "website_url":
                  ?>
                     ux_txt_singal_line_text_<?php echo $controls["timestamp"]; ?> :
                     {
                     required: true,
                             url: true
                     },
               ...
               case "checkbox-list" :
                  ?>
                     "ux_txt_check_box_lists_<?php echo $controls["timestamp"]; ?>[]" :
                     {
                     required: true
                     },
                  <?php
      ...
      switch ($controls["required_type"]) {
         case "enable" :
            switch ($controls["control_type"]) {
               case "website_url" :
                  ?>
                     ux_txt_singal_line_text_<?php echo $controls["timestamp"]; ?>: "<?php echo isset($controls["custom_validation_message"]) && $controls["custom_validation_message"] != "" ? esc_attr($controls["custom_validation_message"]) : "This field is Required!"; ?>",
                  <?php
                  break;
               case "checkbox-list" :
                  ?>
                     "ux_txt_check_box_lists_<?php echo $controls["timestamp"]; ?>[]" : "<?php echo isset($controls["custom_validation_message"]) && $controls["custom_validation_message"] != "" ? esc_attr($controls["custom_validation_message"]) : "This field is Required!"; ?>",
                  <?php
                  break;
               default :
                  ?>
...
if (count($form_unserialized_meta_value["controls"]) > 0) {
   foreach ($form_unserialized_meta_value["controls"] as $controls) {
      if ($controls["control_type"] == "radio-list" || $controls["control_type"] == "checkbox-list") {
         ?>
...
?>
           ...
           {
           jQuery("#form_error_message_frontend_<?php echo $random; ?>").css("display", "none");
           jQuery("#form_success_message_frontend_<?php echo $random; ?>").css("display", "block");
           jQuery("#success_message_text_<?php echo $random; ?>").text("Kindly wait until the form is submitted.");
           }
           jQuery.post(ajaxurl, {
           form_id : form_id,
                   ...
                   {
                   jQuery("#form_success_message_frontend_<?php echo $random; ?>").css("display", "block");
                   overlay_loading_contact_bank_<?php echo $random; ?>();
                   jQuery("#success_message_text_<?php echo $random; ?>").text(form_success_message);
                   setTimeout(function ()
                   {
                   remove_overlay_contact_bank_<?php echo $random; ?>();
                   window.location.href = url_redirect_after_submit;
                   }, 3000);
                   }
                   else
                   {
                   jQuery("#form_error_message_frontend_<?php echo $random; ?>").css("display", "block");
                   }
                   });
           }
   });
...
if (count($form_unserialized_meta_value["controls"]) > 0) {
   foreach ($form_unserialized_meta_value["controls"] as $controls) {
      ?>
         ...
         var control_name = '<?php echo isset($controls["control_type"]) ? esc_attr($controls["control_type"]) : site_url(); ?>';
         ...
         change_label_placement_contact_bank_<?php echo $random; ?>("ux_ddl_label_placement_" + timestamp, timestamp, label_placement_type, control_name);
         apply_input_masking_contact_bank_<?php echo $random; ?>("ux_txt_singal_line_text_" + timestamp, timestamp, input_mask_type, custom_mask);
