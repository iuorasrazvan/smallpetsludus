...
if (!is_user_logged_in()) {
   ...
   if (!$access_granted) {
      ...
      <script type="text/javascript">
         ...
         {
            var premium_edition = <?php echo json_encode($cb_message_premium_edition); ?>;
         ...
         function overlay_loading_contact_bank(message) {
            ...
            var success = <?php echo json_encode($cb_success); ?>;
         ...
         function add_new_form_contact_bank() {
            ...
            jQuery.post(ajaxurl,
            {
               ...
               action: "contact_bank_action_module",
               _wp_nonce: "<?php echo $get_form_id_nonce; ?>"
            },
            function (data)
            {
         ...
         function confirm_delete_contact_bank(meta_id, overlay_message, page_url, param) {
            var checkstr = confirm(<?php echo json_encode($cb_delete_data); ?>);
            if (checkstr === true) {
               jQuery.post(ajaxurl, {
                  meta_id: meta_id,
                  param: param,
         ...
         function delete_radio_options_contact_bank(dynamic_id, dynamicId) {
            ...
            var confirm_delete = confirm(<?php echo json_encode($cb_delete_data); ?>);
         ...
         function add_control_options_contact_bank(dynamicId, type) {
            ...
            if (ddl_options === "" && ddl_values === "") {
               var shortCutFunction = jQuery("#toastTypeGroup_error input:checked").val();
               toastr[shortCutFunction](<?php echo json_encode($cb_add_fill_option_value); ?>);
         ...
         function import_controls_values_contact_bank(dynamicId, type) {
            ...
            {
               var shortCutFunction = jQuery("#toastTypeGroup_error input:checked").val();
               toastr[shortCutFunction](<?php echo json_encode($cb_add_data_to_import); ?>);
            }
      ...
      $licensing_url = $check_contact_bank_wizard == "" ? "cb_wizard_contact_bank" : esc_attr($_GET["page"]);
      if (isset($_GET["page"])) {
         switch ($licensing_url) {
            ...
            case "contact_dashboard" :
               ...
               if (forms_contact_bank === "1") {
                  ?>
                     ...
                     {
                        var shortCutFunction = jQuery("#manage_messages input:checked").val();
                        var $toast = toastr[shortCutFunction](<?php echo json_encode($cb_shortcode_copy_successful); ?>);
                     });
                     clipboard.on("error", function (e)
                     {
                        var shortCutFunction = jQuery("#toastTypeGroup_error input:checked").val();
                        var $toast = toastr[shortCutFunction](<?php echo json_encode($cb_copied_failed) ?>);
                     });
               ...
               if (forms_contact_bank === "1") {
                  ?>
                     ...
                     jQuery(document).ready(function () {
                        var form_redirect_page_url = "<?php echo isset($form_unserialized_meta_value["form_redirect_page_url"]) ? esc_attr($form_unserialized_meta_value["form_redirect_page_url"]) : site_url(); ?>";
                  ...
                  <?php
                  if (isset($_REQUEST["mode"]) && esc_attr($_REQUEST["mode"]) == "edit") {
                     ?>
                  ...
                  if (count($form_unserialized_meta_value["controls"]) > 0) {
                     foreach ($form_unserialized_meta_value["controls"] as $controls) {
                        ?>
                              ...
                              var mathematical_operation_type = '<?php echo isset($values["logical_captcha_mathmatical_operations"]) ? $values["logical_captcha_mathmatical_operations"] : ""; ?>';
                   ...
                   function delete_select_options_contact_bank(dynamic_id, dynamicId) {
                    ...
                    function import_select_options_contact_bank(dynamicId, type) {
                     ...
                     function contact_bank_close_popup_box(random_id) {
                        var confirm_close = confirm(<?php echo json_encode($cb_confirm_close); ?>);
                     ...
                     function number_settings_contact_bank(random_id, event) {
                        ...
                        if (count === 1 && step_count === 1) {
                           jQuery("#ux_text_appear_after_counter_" + random_id).css("display", "block").html(<?php echo json_encode($cb_number_enter_between_message); ?> + min_number + " and " + max_number + <?php echo json_encode($cb_number_increment_by_message); ?> + step_number_cb).addClass("field_label");
                        } else if (step_count === 1) {
                           jQuery("#ux_text_appear_after_counter_" + random_id).css("display", "block").html(<?php echo json_encode($cb_number_increment_by_message); ?> + step_number_cb).addClass("field_label");
                        } else if (count === 1) {
                           jQuery("#ux_text_appear_after_counter_" + random_id).css("display", "block").html(<?php echo json_encode($cb_number_enter_between_message); ?> + min_number + " and " + max_number + "").addClass("field_label");
                        }
                     ...
                     function duplicate_fields_contact_bank(type, last_random_id) {
                        var confirm_duplicate = confirm(<?php echo json_encode($cb_duplicate_control_message); ?>);
                     ...
                     {
                        var controls_type_array = ["text", "paragraph", "select", "checkbox", "checkbox-list", "radio-list", "number", "first_name", "last_name", "email", "website_url", "phone"];
                        if (jQuery.inArray(type, controls_type_array) > -1)
                        {
                           var random_id = new Date().getTime();
                           ...
                           submitHandler: function () {
                              if (jQuery("#ux_div_first_step").hasClass("first-step-helper")) {
                                 if (jQuery(".template-contact-bank").hasClass("cb-active")) {
                                    ...
                                    var form_id = "<?php echo isset($_REQUEST["form_id"]) ? intval($_REQUEST["form_id"]) : "" ?>";
                                    if (template === "layout_settings_blank_form_template") {
                                       ...
                                       if (template === "layout_settings_contact_us_form_template")
                                       {
                                          ...
                                          nonce = "<?php echo $cb_add_form_contact_us_template_nonce; ?>";
                                    ...
                                    var shortCutFunction = jQuery("#toastTypeGroup_error input:checked").val();
                                    toastr[shortCutFunction](<?php echo json_encode($cb_select_form_message); ?>);
                                 }
                              } else {
                                 if (window.CKEDITOR) {
                                 ...
                                 var template = jQuery(".cb-active").attr("value");
                                 var form_id = "<?php echo isset($_REQUEST["form_id"]) ? intval($_REQUEST["form_id"]) : 0; ?>";
                                 submit_handler_common_contact_bank(form_id, "#ux_frm_add_new_forms", "", "add_form_module", "<?php echo $add_new_form_module; ?>", <?php echo json_encode($cb_add_form_message); ?>, "contact_dashboard", template, contact_bank_control_ids);
                              }
                           }
                        });
                     }
            ...
            case "cb_layout_settings" :
               ...
               if (layout_settings_contact_bank === "1") {
                  ...
                  ?>
                        jQuery("#ux_ddl_choose_template").val("<?php echo isset($_REQUEST["form_id"]) ? intval($_REQUEST["form_id"]) : ""; ?>");
                        ...
                        jQuery("#ux_ddl_form_design_title_font_family").val("<?php echo(isset($layout_settings_data["layout_settings_form_design_title_font_family"])) ? htmlspecialchars_decode($layout_settings_data["layout_settings_form_design_title_font_family"]) : "Roboto Condensed"; ?>");
                        ...
                        jQuery("#ux_ddl_form_design_description_font_family").val("<?php echo(isset($layout_settings_data["layout_settings_form_design_description_font_family"])) ? htmlspecialchars_decode($layout_settings_data["layout_settings_form_design_description_font_family"]) : "Roboto Condensed"; ?>");
                        ...
                        jQuery("#ux_ddl_input_field_font_family").val("<?php echo(isset($layout_settings_data["layout_settings_input_field_font_family"])) ? htmlspecialchars_decode($layout_settings_data["layout_settings_input_field_font_family"]) : "Roboto Condensed"; ?>");
                        ...
                        jQuery("#ux_ddl_button_title_font_family").val("<?php echo isset($layout_settings_data["layout_settings_button_font_family"]) ? htmlspecialchars_decode($layout_settings_data["layout_settings_button_font_family"]) : "Roboto Condensed"; ?>");
                        jQuery("#ux_ddl_label_field_font_family").val("<?php echo(isset($layout_settings_data["layout_settings_label_field_font_family"])) ? htmlspecialchars_decode($layout_settings_data["layout_settings_label_field_font_family"]) : "Roboto Condensed"; ?>");
                        jQuery("#ux_ddl_contact_messages_font_family").val("<?php echo(isset($layout_settings_data["layout_settings_messages_font_family"])) ? htmlspecialchars_decode($layout_settings_data["layout_settings_messages_font_family"]) : "Roboto Condensed"; ?>");
                        ...
                        jQuery("#ux_ddl_error_messages_font_family").val("<?php echo(isset($layout_settings_data["layout_settings_error_messages_font_family"])) ? htmlspecialchars_decode($layout_settings_data["layout_settings_error_messages_font_family"]) : "Roboto Slab"; ?>");
                     });
               ...
               if (custom_css_contact_bank === "1") {
                  ?>
                     jQuery("#ux_frm_custom_css").validate({
                        submitHandler: function () {
                           submit_handler_common_contact_bank("", "#ux_frm_custom_css", "", "custom_css_module", "<?php echo $custom_css_nonce; ?>", <?php echo json_encode($cb_custom_css_message); ?>, "cb_custom_css", "", "");
                        }
                     });
                     var sidebar_load_interval = setInterval(load_sidebar_content_contact_bank, 1000);
                     setTimeout(function () {
            ...
            case "cb_email_templates" :
               ...
               if (email_templates_contact_bank === "1") {
                  ?>
                     ...
                     jQuery(document).ready(function () {
                        jQuery("#ux_ddl_forms").val("<?php echo isset($_REQUEST["form_id"]) ? esc_attr($_REQUEST["form_id"]) : ""; ?>");
                        jQuery("#ux_ddl_email_templates").val("<?php echo isset($_REQUEST["template_type"]) ? esc_attr($_REQUEST["template_type"]) : "form_admin_notification_email"; ?>");
                     ...
                     jQuery("#ux_frm_manage_email_templates").validate({
                        ...
                        submitHandler: function () {
                           ...
                           {
                              var shortCutFunction = jQuery("#toastTypeGroup_error input:checked").val();
                              toastr[shortCutFunction](<?php echo json_encode($cb_select_form_error_message); ?>);
                           } else
                           {
                              if (window.CKEDITOR) {
                              ...
                              var form_id = "<?php echo isset($_REQUEST["form_id"]) ? intval($_REQUEST["form_id"]) : 0; ?>";
                              var template_type = "<?php echo isset($_REQUEST["template_type"]) ? esc_attr($_REQUEST["template_type"]) : "form_admin_notification_email"; ?>";
                              submit_handler_common_contact_bank("", "#ux_frm_manage_email_templates", "", "email_templates_module", "<?php echo $email_templates_nonce; ?>", <?php echo json_encode($cb_email_templates_save); ?>, "cb_email_templates&form_id=" + form_id + "&template_type=" + template_type, "", "");
                           }
                        }
                     });
                  <?php
                  ...
                  ?>
                     ...
                     jQuery("#ux_frm_general_settings").validate({
                        submitHandler: function (form) {
                           submit_handler_common_contact_bank("", "#ux_frm_general_settings", "", "general_settings_module", "<?php echo $general_settings_nonce; ?>",<?php echo json_encode($cb_update_general_settings_data); ?>, "cb_general_settings", "");
                        }
                     });
                  <?php
               }
               ...
               if (submissions_contact_bank === "1") {
                  ?>
                     ...
                     jQuery(document).ready(function ()
                     {
                        ...
                        jQuery("#ux_ddl_manage_email").val("<?php echo isset($_REQUEST["form_id"]) ? intval($_REQUEST["form_id"]) : "" ?>");
                  <?php
                  if (isset($_REQUEST["form_id"])) {
                     ?>
