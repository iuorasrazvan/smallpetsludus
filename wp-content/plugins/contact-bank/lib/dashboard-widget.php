...
function get_count_of_data_contact_bank($type) {
   ...
   $cb_total_data_count = $wpdb->get_var
       ...
       $wpdb->prepare
           (
           "SELECT count(meta_id) FROM " . contact_bank_meta() . " WHERE meta_key = %s", $type
       )
   );
...
$cb_total_controls = $wpdb->get_results
    ...
    $wpdb->prepare
        (
        "SELECT *  FROM " . contact_bank_meta() . " WHERE meta_key = %s", "form_data"
    )
);
...
<ul class="cb-statistics-list">			
   <li class="cb-forms-added">
      <a href="admin.php?page=contact_dashboard">
         <strong><?php echo get_count_of_data_contact_bank("form_data"); ?> Forms</strong>		
      </a>
   </li>
   <li class="cb-controls-added">
      <a href="admin.php?page=contact_dashboard">
         <strong><?php echo $forms_controls_count; ?> Controls</strong>			
      </a>
   </li>
   <li class="cb-form-submissions">
      <a href="admin.php?page=cb_submissions">
         <strong><?php echo get_count_of_data_contact_bank("submission_form_data"); ?> Form Submissions</strong>		
      </a>
   </li>
