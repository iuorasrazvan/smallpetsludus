...
if (!is_user_logged_in()) {
   ...
   if (!current_user_can("manage_options")) {
      ...
      /*
        Class Name: dbHelper_install_script_contact_bank
        Parameter: No
        Description: This Class is used for Insert,Update and Delete operations.
        Created On: 29-05-2017 10:02
      ...
      /*
        Function Name: contact_bank_main_table
        Parameter: No
      ...
      /*
        Function Name: contact_bank_meta_table
        Parameter: No
      ...
      if (!function_exists("contact_bank_meta_table")) {
         function contact_bank_meta_table() {
            ...
            $parent_table_data = $wpdb->get_results
                (
                "SELECT id,type FROM " . contact_bank()
            );
            ...
            foreach ($parent_table_data as $row) {
               switch ($row->type) {
                  case "custom_css":
                     ...
                     $insert_custom_css_array["meta_key"] = "custom_css";
                     $insert_custom_css_array["meta_value"] = serialize($custom_css_data_array);
                     $obj_dbHelper_contact_bank->insertCommand(contact_bank_meta(), $insert_custom_css_array);
                     break;
                  case "general_settings":
                     $general_settings_data_array = array();
                     $general_settings_data_array["remove_tables_at_uninstall"] = "disable";
                     ...
                     $insert_general_settings_array["meta_key"] = "general_settings";
                     $insert_general_settings_array["meta_value"] = serialize($general_settings_data_array);
                     $obj_dbHelper_contact_bank->insertCommand(contact_bank_meta(), $insert_general_settings_array);
                     break;
                  ...
                  case "roles_and_capabilities":
                     ...
                     $roles_data_serialize["meta_key"] = "roles_and_capabilities";
                     $roles_data_serialize["meta_value"] = serialize($roles_data);
                     $obj_dbHelper_contact_bank->insertCommand(contact_bank_meta(), $roles_data_serialize);
                     break;
               }
            }
         }
      ...
      switch ($contact_bank_version_number) {
         case "":
            ...
            $form_layout_settings["meta_key"] = "layout_settings";
            $form_layout_settings["old_form_id"] = $parent_last_id;
            $form_layout_settings["meta_value"] = maybe_serialize($layout_settings_data_array);
            $obj_dbHelper_contact_bank->insertCommand(contact_bank_meta(), $form_layout_settings);
            break;
         default :
            global $wpdb, $current_user;
            if (count($wpdb->get_var("SHOW TABLES LIKE '" . $wpdb->prefix . "cb_contact_form'")) != 0 and count($wpdb->get_var("SHOW TABLES LIKE '" . $wpdb->prefix . "cb_form_settings_table'")) != 0 and count($wpdb->get_var("SHOW TABLES LIKE '" . $wpdb->prefix . "cb_create_control_form'")) != 0) {
               contact_bank_main_table();
               contact_bank_meta_table();
               // fetch form_ids of all forms
               $get_contact_form = $wpdb->get_results
                   (
                   "SELECT * FROM " . $wpdb->prefix . "cb_contact_form"
               );
               if (count($get_contact_form) > 0) {
                  foreach ($get_contact_form as $value) {
                     ...
                     $get_contact_form_data = $wpdb->get_results
                         (
                         $wpdb->prepare
                             (
                             "SELECT * FROM " . $wpdb->prefix . "cb_form_settings_table where form_id = %d", $form_id
                         )
                     );
                     ...
                     foreach ($get_contact_form_email_template_data as $email_template_data) {
                        if ($email_template_count == 0) {
                           ...
                           $admin_email_template_form_data["template_send_to"] = $email_template_data->send_to == 0 ? "send_to_email" : "select_field";
                           ...
                           $client_email_template_form_data["template_send_to"] = $email_template_data->send_to == 0 ? "send_to_email" : "select_field";
                     ...
                     if (count($get_contact_form_data) > 0) {
                        ...
                        $cb_form_data_array["form_redirect"] = $form_data_array["redirect"] == 0 ? "page" : "url";
                        $cb_form_data_array["form_redirect_url"] = $form_data_array["redirect"] == 1 ? $form_data_array["redirect_url"] : "";
                        $cb_form_data_array["form_redirect_page_url"] = $form_data_array["redirect"] == 0 ? $form_data_array["redirect_url"] : "";
                        ...
                        if (count($cb_create_control_form) > 0) {
                           ...
                           foreach ($cb_create_control_form as $value) {
                              ...
                              if (count($cb_controls_data) > 0) {
                                 ...
                                 $cb_form_controls_array["label_name"] = $cb_controls_data["cb_label_value"] != "" ? $cb_controls_data["cb_label_value"] : "Untitled";
                                 $cb_form_controls_array["required_type"] = $cb_controls_data["cb_control_required"] == 1 ? "enable" : "disable";
                                 ...
                                 switch ($field_id) {
                                    ...
                                    case "4":
                                       ...
                                       $cb_select_control_array = unserialize($cb_controls_data["cb_dropdown_option_val"]);
                                       $option_values = '[';
                                       foreach ($cb_select_control_array as $value) {
                                          $option_values .= '{"value":"' . $value . '","text":"' . $value . '"},';
                                       }
                                       if (substr($option_values, -1, 1) == ',') {
                                    ...
                                    case "5":
                                       ...
                                       $cb_checkbox_list_array = unserialize($cb_controls_data["cb_checkbox_option_val"]);
                                       $option_values = '[';
                                    ...
                                    case "6":
                                       ...
                                       $cb_radio_list_array = unserialize($cb_controls_data["cb_radio_option_val"]);
                                       $option_values = '[';
                                    ...
                                    case "9":
                                       ...
                                       $cb_form_controls_array["multiple_upload"] = $cb_controls_data["cb_allow_multiple_file"] == 1 ? "true" : "false";
                                    ...
                                    case "13":
                                       ...
                                       $cb_form_controls_array["time_format"] = $cb_controls_data["cb_hour_format"] == 12 ? "12hour" : "24hour";
                        ...
                        $add_form_meta_data["meta_id"] = $parent_last_id;
                        $add_form_meta_data["meta_key"] = "form_data";
                        $add_form_meta_data["meta_value"] = maybe_serialize($cb_form_data_array);
                        $obj_dbHelper_contact_bank->insertCommand(contact_bank_meta(), $add_form_meta_data);
                        ...
                        $add_form_layout_settings_meta_data["meta_key"] = "layout_settings";
                        $add_form_layout_settings_meta_data["meta_value"] = maybe_serialize($layout_settings_array);
                        $obj_dbHelper_contact_bank->insertCommand(contact_bank_meta(), $add_form_layout_settings_meta_data);
                           ...
                           foreach ($unique_form_submit_id as $key) {
                              ...
                              foreach ($cb_front_end_data_array as $id => $label_value) {
                                 ...
                                 $cb_selected_form_data = $wpdb->get_var
                                     ...
                                     $wpdb->prepare
                                         (
                                         "SELECT meta_value FROM " . contact_bank_meta() . " WHERE meta_id = %d and meta_key = %s", $parent_last_id, "form_data"
                                     )
                                 );
                                 ...
                                 if (isset($cb_unserialize_selected_form_data["controls"]) && count($cb_unserialize_selected_form_data["controls"]) > 0) {
                                    foreach ($cb_unserialize_selected_form_data["controls"] as $control_value) {
                                       if ($control_value["label_name"] == $cb_dynamic_settings_value) {
                              ...
                              $frontend_form_meta_data["meta_key"] = "submission_form_data";
                              $frontend_form_meta_data["meta_value"] = maybe_serialize($cb_label_value_array);
                              $obj_dbHelper_contact_bank->insertCommand(contact_bank_meta(), $frontend_form_meta_data);
                           }
                        }
                     }
                  }
      ...
      update_option("contact-bank-version-number", "3.0");
   }
}