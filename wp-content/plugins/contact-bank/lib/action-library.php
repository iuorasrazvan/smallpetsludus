...
if (!defined("ABSPATH")) {
    ...
    if (!$access_granted) {
        ...
        function get_contact_bank_details_date($meta_key, $date1, $date2, $form_id) {
			...
			WHERE meta_key = %s and old_form_id = %d ORDER BY id DESC", $meta_key, $form_id
                    )
            );
        ...
        if (isset($_REQUEST["param"])) {
            ...
            switch (esc_attr($_REQUEST["param"])) {
                case "wizard_contact_bank":
                    if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : "", "contact_bank_check_status")) {
                ...
                case "cb_get_form_id_contact_bank":
                    if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : "", "cb_get_form_id_nonce")) {
                        global $wpdb;
                        $parent_id = $wpdb->get_var
                                (
                                $wpdb->prepare
                                        (
                                        "SELECT id FROM " . contact_bank() . "
							WHERE type = %s", "forms"
                                )
                        );
                        ...
                        $parent_last_id = $obj_dbHelper_contact_bank->insertCommand(contact_bank(), $add_form_parent_data);
                        ...
                        $add_form_meta_data["meta_key"] = "form_data";
                        $add_form_meta_data["meta_value"] = maybe_serialize($add_form_record);
                        $obj_dbHelper_contact_bank->insertCommand(contact_bank_meta(), $add_form_meta_data);
                        ...
                        $add_form_meta_data = array();
                        $add_form_meta_data["meta_id"] = $parent_last_id;
                        $add_form_meta_data["meta_key"] = "layout_settings";
                        $add_form_meta_data["old_form_id"] = $parent_last_id;
                        $add_form_meta_data["meta_value"] = maybe_serialize($layout_settings_array);
                        $obj_dbHelper_contact_bank->insertCommand(contact_bank_meta(), $add_form_meta_data);
                ...
                case "add_form_module" :
                    if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : "", "add_new_form_module")) {
                        parse_str(isset($_REQUEST["data"]) ? base64_decode($_REQUEST["data"]) : "", $add_form_data);
                        $id = isset($_REQUEST["form_meta_id"]) ? intval($_REQUEST["form_meta_id"]) : 0;
                        $control_ids = isset($_REQUEST["control_ids"]) ? array_map("esc_attr", is_array(json_decode($_REQUEST["control_ids"])) ? json_decode($_REQUEST["control_ids"]) : array()) : array();
                        ...
                        $contact_meta_value = $wpdb->get_var
                                ...
                                $wpdb->prepare
                                        ...
                                        " WHERE meta_id = %d and meta_key = %s", $id, "form_data"
                                )
                        );
                        ...
                        foreach ($control_ids as $control_id) {
                            ...
                            $add_form_controls_data["timestamp"] = $control_id;
                            ...
                            if ($add_form_controls_data["control_type"] != "html" && $add_form_controls_data["control_type"] != "divider" && $add_form_controls_data["control_type"] != "logical-captcha" && $add_form_controls_data["control_type"] != "recaptcha" && $add_form_controls_data["control_type"] != "logical-captcha" && $add_form_controls_data["control_type"] != "credit_card" && $add_form_controls_data["control_type"] != "section_break" && $add_form_controls_data["control_type"] != "anti_spam") {
                                $form_controls .= "<p><strong>" . $template_label_name . "</strong>: " . "[control_" . $add_form_controls_data["timestamp"] . "]</p>";
                            } else if ($add_form_controls_data["control_type"] == "credit_card") {
                                $form_controls .= "<p><strong> Credit Card Number </strong>: " . "[control_card_number_" . $add_form_controls_data["timestamp"] . "]</p>";
                                $form_controls .= "<p><strong> Card Expiry Date </strong>: " . "[control_expiry_date_" . $add_form_controls_data["timestamp"] . "]</p>";
                                $form_controls .= "<p><strong> CVV Number </strong>: " . "[control_cvv_number_" . $add_form_controls_data["timestamp"] . "]</p>";
                            }
                        ...
                        $add_form_record["layout_settings_template"] = isset($_REQUEST["template"]) ? sanitize_text_field($_REQUEST["template"]) : "layout_settings_blank_form_template";
                        ...
                        if ($id == 0) {
                            ...
                            $where["meta_key"] = "form_data";
                            $add_form_meta_data["meta_value"] = maybe_serialize($add_form_record);
                            $obj_dbHelper_contact_bank->updateCommand(contact_bank_meta(), $add_form_meta_data, $where);
                        }
                    }
                    break;

                case "cb_add_form_contact_us_template" :
                    if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : "", "cb_add_form_contact_us_template_nonce")) {
                        $form_id = isset($_REQUEST["form_meta_id"]) ? intval($_REQUEST["form_meta_id"]) : "";
                        ...
                        $contact_meta_value = $wpdb->get_var
                                ...
                                $wpdb->prepare
                                        (
                                        "SELECT meta_value from " . contact_bank_meta() .
                                        " WHERE meta_id = %d and meta_key = %s", $form_id, "form_data"
                                )
                        );
                        ...
                        $control_timestamp = "<p>Hello Admin,</p><p>A new user visited your website.</p><p>Here are the details :</p>" . $form_controls . "<p>Thanks,</p><p><strong>Technical Support Team</strong></p><p><strong>" . $site_title_name . "</strong></p>";
                        $form_unserialized_data["layout_settings_template"] = isset($_REQUEST["template"]) ? sanitize_text_field($_REQUEST["template"]) : "layout_settings_blank_form_template";
                        ...
                        $where["meta_key"] = "form_data";
                        $add_form_meta_data["meta_value"] = maybe_serialize($form_unserialized_data);
                        $obj_dbHelper_contact_bank->updateCommand(contact_bank_meta(), $add_form_meta_data, $where);
                    }
                    break;

                case "delete_submission_module":
                    if (wp_verify_nonce((isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : ""), "cb_submission_single_delete")) {
                        $id = isset($_REQUEST["meta_id"]) ? intval(stripcslashes($_REQUEST["meta_id"])) : "";
                        $delete_submission_data = array();
                        $delete_submission_data["id"] = $id;
                        $obj_dbHelper_contact_bank->deleteCommand(contact_bank_meta(), $delete_submission_data);
                    }
                    break;
                case "delete_contact_forms":
                    if (wp_verify_nonce((isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : ""), "contact_forms_delete_nonce")) {
                        $meta_id = isset($_REQUEST["meta_id"]) ? intval(stripcslashes($_REQUEST["meta_id"])) : "";
                        $delete_form_data = array();
                        $delete_form_data_parent = array();
                        $delete_form_data["meta_id"] = $meta_id;
                        $delete_form_data_parent["id"] = $meta_id;
                        $obj_dbHelper_contact_bank->deleteCommand(contact_bank(), $delete_form_data_parent);
                        $obj_dbHelper_contact_bank->deleteCommand(contact_bank_meta(), $delete_form_data);
                    }
                    break;
                ...
                case "custom_css_module":
                    if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : "", "custom_css_nonce")) {
                        ...
                        $obj_dbHelper_contact_bank->updateCommand(contact_bank_meta(), $update_custom_css_array, $where);
                    }
                    break;

                case "email_templates_module" :
                    if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : "", "email_templates_nonce")) {
                        parse_str(isset($_REQUEST["data"]) ? base64_decode($_REQUEST["data"]) : "", $email_templates_data);
                        ...
                        $form_data = $wpdb->get_var
                                ...
                                $wpdb->prepare
                                        (
                                        "SELECT meta_value FROM " . contact_bank_meta() . " WHERE meta_key = %s and meta_id = %d", "form_data", $form_id
                                )
                        );

                        $unserialized_data_forms = unserialize($form_data);
                        $unserialized_data_forms[$template_type] = $email_templates_array;

                        $email_template_update_data = array();
                        $email_template_update_data["meta_value"] = maybe_serialize($unserialized_data_forms);
                        $where = array();
                        $where["meta_key"] = "form_data";
                        $where["meta_id"] = $form_id;
                        $obj_dbHelper_contact_bank->updateCommand(contact_bank_meta(), $email_template_update_data, $where);
                    }
                    break;

                case "general_settings_module":
                    if (wp_verify_nonce((isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : ""), "general_settings_nonce")) {
                        parse_str((isset($_REQUEST["data"]) ? base64_decode($_REQUEST["data"]) : ""), $general_settings_data);
                        ...
                        $update_general_settings["recaptcha_private_key"] = sanitize_text_field($general_settings_data["ux_txt_recaptcha_private_key"]);
                        $update_general_settings["recaptcha_public_key"] = sanitize_text_field($general_settings_data["ux_txt_recaptcha_public_key"]);

                        $where = array();
                        $update_general_settings_data = array();
                        $where["meta_key"] = "general_settings";
                        $update_general_settings_data["meta_value"] = serialize($update_general_settings);
                        $obj_dbHelper_contact_bank->updateCommand(contact_bank_meta(), $update_general_settings_data, $where);
                    }
                    break;
            }
            die();
