...
/*,	_checkDefValueInp: function( html ) {
		if(typeof(cfsAllCountries) == 'undefined') return;
		var $inp = this._$editWnd.find('[name="value"]')
		,	state = $inp.tagName().toLowerCase() == 'input' ? 'text' : 'select';
		if(state == 'select' && $inp.attr('multiple')) {
			state = 'select_mult';
		}
		switch(html) {
			case 'countryList': case 'countryListMultiple':
				if(state == 'text') {
					var $select = jQuery('<select name="value" />');
					$select.insertAfter( $inp );
					$inp.remove();
					if(html == 'countryListMultiple') {
						$select.attr('multiple', 'multiple');
					}
					for(var id in cfsAllCountries) {
						$select.append('<option value="'+ id+ '">'+ cfsAllCountries[ id ]+ '</option>');
					}
				}
				break;
			default:
				if(state != 'text') {
					var $txt = jQuery('<input type="text" name="value" />');
					$txt.insertAfter( $inp );
					$inp.remove();
				} 
				break;
